...
type Compiler struct {
	...
	compiledModules map[string]*common.CompiledFunction
...
func NewCompiler(
	...
	return &Compiler{
		...
		trace:           trace,
		modules:         modules,
		compiledModules: make(map[string]*common.CompiledFunction),
	}
}
...
func (c *Compiler) Compile(node parser.Node) error {
	...
	case *parser.IntLit:
		c.emit(node, parser.OpConstant,
			c.addConstant(&common.Int{Value: node.Value}))
	case *parser.FloatLit:
		c.emit(node, parser.OpConstant,
			c.addConstant(&common.Float{Value: node.Value}))
	case *parser.BoolLit:
		if node.Value {
	...
	case *parser.StringLit:
		if len(node.Value) > common.MaxStringLen {
			return c.error(node, common.ErrStringLimit)
		}
		c.emit(node, parser.OpConstant,
			c.addConstant(&common.String{Value: node.Value}))
	case *parser.CharLit:
		c.emit(node, parser.OpConstant,
			c.addConstant(&common.Char{Value: node.Value}))
	...
	case *parser.MapLit:
		for _, elt := range node.Elements {
			// key
			if len(elt.Key) > common.MaxStringLen {
				return c.error(node, common.ErrStringLimit)
			}
			c.emit(node, parser.OpConstant,
				c.addConstant(&common.String{Value: elt.Key}))
	...
	case *parser.FuncLit:
		...
		compiledFunction := &common.CompiledFunction{
	...
	case *parser.ImportExpr:
		...
		if mod := c.modules.Get(node.ModuleName); mod != nil {
			...
			case common.Object: // builtin module
...
func (c *Compiler) loadCompiledModule(
	modulePath string,
) (mod *common.CompiledFunction, ok bool) {
	if c.parent != nil {
...
func (c *Compiler) storeCompiledModule(
	modulePath string,
	module *common.CompiledFunction,
) {
	if c.parent != nil {
...
func (c *Compiler) addConstant(o common.Object) int {
	if c.parent != nil {
